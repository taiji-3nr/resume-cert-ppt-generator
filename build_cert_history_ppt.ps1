...
$hasProjectImage = Test-Path $projectImage
...
$slides = @()
$slide1Bg = if ($hasProjectImage) { Picture 2 'generated certification history background' 'rId2' 0 0 $W $H } else { Shape 2 'background' 0 0 $W $H 'F4F8FA' $null '' 100 'FFFFFF' }
$slides += SlideXml (($slide1Bg) +
...
$body5 = if ($hasProjectImage) { Picture 2 'generated certification history background' 'rId2' 0 0 $W $H } else { Shape 2 'background' 0 0 $W $H 'F4F8FA' $null '' 100 'FFFFFF' }
...
for ($i=1; $i -le $slides.Count; $i++) {
  ...
  Write-Utf8 (Join-Path $workDir "ppt\slides\_rels\slide$i.xml.rels") (SlideRels ($hasProjectImage -and ($i -eq 1 -or $i -eq 5)))
